...
void handleInput() {
    if (glfwGetKey(window, GLFW_KEY_W)) cameraZoom -= 0.4;
    else if (glfwGetKey(window, GLFW_KEY_S)) cameraZoom += 0.4;
...
bool initializeWindow() {
    ...
    window = glfwCreateWindow(1024, 768, "Mars Model", nullptr, nullptr);
    if (window == nullptr) {
        fprintf(stderr,
...
bool updateMVPTransformation()
{
    ...
    //translation
    model = glm::translate(model, glm::vec3(curr_x, curr_y, 0.0f));
...
bool initializeVertexBuffer() {
    ...
    renderingObject.SetTexture(uvBufferData, "../resources/planetMars.bmp");
